...
function Ottoman(options) {
  ...
  Object.defineProperty(this, 'bucket', {
    ...
    set: function (bucket) {
      this.store = new StoreAdapter.Couchbase(bucket, this.typeKey);
    }
  });
...
Ottoman.prototype._createSchema = function (name, schemaDef, options) {
  ...
  if (options.id === undefined) {
    if (!schema.field(this.idKey)) {
      var idField = this._makeField(this.idKey, {
        type: 'string',
        auto: 'uuid',
        readonly: true
      });
...
Ottoman.prototype._countModels = function (model, filter, options, callback) {
  ...
  for (var i in options) {
    if (options.hasOwnProperty(i)) {
      if (i.toLowerCase() === 'union') {
        countOpts.union = this._normFilter(options.union);
      } else {
...
Ottoman.prototype._findModels = function (model, filter, options, callback) {
  ...
  for (var i in options) {
    if (options.hasOwnProperty(i)) {
      if (i.toLowerCase() === 'union') {
        findOpts.union = this._normFilter(options.union);
      } else {
...
Ottoman.prototype.fromCoo = function (data, type) {
  ...
  if (data[this.typeKey]) {
    if (type) {
      if (type !== data[this.typeKey]) {
