...
function _decodeValue(context, type, data) {
  if (data instanceof Object && data.$ref) {
    ...
    if ((modelType.type && modelType.type === 'Mixed') ||
      ...
      modelType = context.typeByName(data[context.typeKey]);

      if (!modelType) {
        throw new Error('Invalid type in mixed reference ('
          + data[context.typeKey] + ')');
      }
    }

    return modelType.ref(data.$ref);
  } else if (data instanceof Object && data[context.typeKey]) {
    if (type === mixedCoreType) {
      type = context.typeByName(data[context.typeKey]);
      if (!type) {
    ...
    if (type instanceof CoreType) {
      ...
      for (var i = 0; i < data.length; ++i) {
        outArr[i] = _decodeValue(context, type.type, data[i], context.typeKey);
      }
      return outArr;
