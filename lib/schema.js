...
function _decodeValue(context, type, data, typeKey) {
  if (data instanceof Object && data.$ref) {
    ...
    if ((modelType.type && modelType.type === 'Mixed') ||
      ...
      modelType = context.typeByName(data[typeKey]);
  ...
  } else if (data instanceof Object && data[typeKey]) {
    if (type === mixedCoreType) {
      type = context.typeByName(data[typeKey]);
      if (!type) {
    ...
    if (type instanceof CoreType) {
      ...
      for (var i = 0; i < data.length; ++i) {
        outArr[i] = _decodeValue(context, type.type, data[i], typeKey);
      }
      return outArr;
      ...
      _decodeFields(context, type.fields, outObj, data, typeKey);
...
function _decodeFields(context, fields, obj, data, typeKey) {
  for (var i in data) {
    if (data.hasOwnProperty(i)) {
      ...
      obj[i] = _decodeValue(context, field.type, data[i], typeKey);
    }
  }
}
...
Schema.prototype.applyDataToObject = function (obj, data) {
  _decodeFields(this.context, this.fields, obj, data, this.typeKey);
};
