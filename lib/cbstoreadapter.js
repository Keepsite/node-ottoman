 ...
 * @constructor
 */
function CbStoreAdapter(bucket, typeKey, cb) {
 ...
 * @ignore
 */
function _buildViewMapFn(schemaName, fields, typeKey) {
  var out = '';
  out += 'function(doc, meta) {\n';
  out += '  if(doc.' + typeKey + ' === \'' + schemaName + '\') {\n';
...
CbStoreAdapter.prototype._createViewIndex =
  function (modelName, name, fields, callback) {
    var mapFnStr = _buildViewMapFn(modelName, fields, this.typeKey);
...
CbStoreAdapter.prototype._ensureMrIndices = function (callback) {
  ...
  ddocs.push({
    ...
    data: {
      views: {
        count_items: {
          map: 'function(doc,meta){emit(doc.' + this.typeKey + ',null);}',
          reduce: '_count'
        }
      }
    }
...
function _buildFilterExprs(filters, expressions, root) {
  var SPECIAL_KEYS = ['$exists', '$contains', '$like', '$missing'];
  var BOOLEAN = ['or', 'and'];
  if (!root) {
    root = '';
  }
      ...
      if (BOOLEAN.indexOf(i.toLowerCase()) !== -1) {
        ...
        for (var j in filters[i]) {
          if (filters[i].hasOwnProperty(j)) {
            if (filters[i][j].constructor === Array) {
              for (var k = 0; k < filters[i][j].length; k++) {
                _buildFilterExprs(
                  filters[i][j][k],
                  booleanExprs,
                  '`' + j + '`.'
        ...
        if (typeof filters[i] === 'number' || typeof filters[i] === 'boolean') {
          ...
          if (keys.length === 1 && SPECIAL_KEYS.includes(keys[0])) {
            continue;
          }
...
CbStoreAdapter.prototype.count = function (type, modelName, options, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var expressions = [];
    expressions.push('`' + self.typeKey + '`=\'' + modelName + '\'');
    if (options.filter) {
...
CbStoreAdapter.prototype.find = function (type, modelName, options, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (options.union !== undefined) {
      expressions = [];
      expressions.push('`' + self.typeKey + '`=\'' + modelName + '\'');
      _buildFilterExprs(options.union, expressions);
      if (expressions.length > 0) {
        unionQs = ' UNION ( SELECT META(b).id AS id FROM ' + bucketName +
        ' b WHERE ' + expressions.join(' AND ') +')';
      }
    }
